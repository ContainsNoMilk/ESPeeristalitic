...
void setup() {
  ...
  server.on("/", [](){ server.send(200,"text/html; charset=UTF-8", createHomePage()); });
  server.on("/manual", [](){ server.send(200,"text/html; charset=UTF-8", createManualPage()); });
  server.on("/calibration", [](){ server.send(200,"text/html; charset=UTF-8", createCalibrationPage()); });
  server.on("/programs", [](){ server.send(200,"text/html; charset=UTF-8", createProgramsPage()); });

  // AJAX Endpoints wie zuvor
  server.on("/get_pumps", [](){
    String json = "[";
    for(int i=0;i<4;i++){
      if(i>0) json += ",";
...
server.on("/stop_calibration", [](){
    ...
    float rate = 100.0 / durationSec;
    updatePumpFlowRate(p, rate);
    server.send(200,"text/plain","Kalibrierung für Pumpe "+String(p+1)+" gestoppt. Dauer: "+String(durationSec,2)+" s. Rate: "+String(rate,2)+" ml/s.");
});
  ...
  server.on("/toggle_program", [](){
    ...
    server.send(200,"text/plain","Programm "+String(idx+1)+" wurde "+status+".");
  });
  ...
  server.on("/add_program", HTTP_POST, [](){
    if(!server.hasArg("days") || !server.hasArg("interval") || !server.hasArg("time") || !server.hasArg("amount") || !server.hasArg("pump")) {
    ...
    prog.active = false;
    prog.pumpIndex = server.arg("pump").toInt();
    prog.lastRun = 0;
    addProgram(prog);
